...
class UserDetailSection(Vertical):
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalScroll(id='detail-content', classes='hidden'):
            ...
            with Horizontal(id='fetch-release-bar'):
                ...
                yield Button(
                    'Fetch releases', id='fetch-release-button', variant='primary'
                )
    ...
    def show_user(
        ...
        if artists:
            artist_select.set_options(
                [(name, id_artist) for id_artist, name in artists]
            )
        else:
            artist_select.set_options([])
    ...
    def _toggle_fetch(self, fetch_button_id: str, label: str, fetching: bool) -> None:
    ...
    def _submit_settings(self) -> None:
        ...
        try:
            ...
            releases_not_before = (
                date.fromisoformat(rnb_str) if rnb_str else date.today()
            )
    ...
    def _request_fetch_releases(self) -> None:
        ...
        self.query_one(FetchReleases).reset()
        self.post_message(
            self.FetchReleasesRequested(
                self._lastfm_username, password, artist_select.value
            )
        )
